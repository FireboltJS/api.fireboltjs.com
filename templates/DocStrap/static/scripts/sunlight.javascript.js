(function(sunlight, undefined){
	...
	sunlight.registerLanguage("javascript", {
		...
		customTokens: {
			...
			globalFunction: {
				values: [
					"encodeURI", "encodeURIComponent", "decodeURI", "decodeURIComponent", "parseInt", "parseFloat", "isNaN", "isFinite", "eval",
		...
		customParseRules: [
			...
			function(context) {
				...
				if (context.reader.current() !== "/" || peek === "/" || peek === "*" || peek === "\n") {
				...
				while (context.reader.peek() !== context.reader.EOF) {
					if (!/[gim]/.test(context.reader.peek())) {
						break;
					}
